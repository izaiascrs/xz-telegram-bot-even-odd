...
import { MoneyManager } from "../money-management/money-manager-v2";
import { CompleteBackTestResult, Strategy, Ticks, TradeSignal } from "./types";
...
const countStreaks = (streaks: number[]) =>
  streaks.reduce(
    (acc, streak) => ({ ...acc, [streak]: (acc[streak] || 0) + 1 }),
...
function calculateDigitStats(
  ...
  const targetTrades = recentTrades.filter(
    (trade) => Math.floor(digits[trade.position]) === targetDigit
  );

...
function seed() {
    return __awaiter(this, void 0, void 0, function* () {
        const db = (0, schema_1.initDatabase)();
        ...
        const tradesByDateAndHour = new Map();
        ...
        flatTrades.forEach(trade => {
            ...
            (_a = hourMap.get(trade.hour)) === null || _a === void 0 ? void 0 : _a.push(trade);
        });
        ...
        for (const [date, hourMap] of tradesByDateAndHour) {
            for (const [hour, hourTrades] of hourMap) {
                for (const trade of hourTrades) {
                    yield insertTrade(db, trade);
                }
                yield updateHourlyStats(db, date, hour, hourTrades);
            }
        }
        // Criar sequências de exemplo
        yield createSequences(db);
        console.log('Seed concluído com sucesso!');
        process.exit(0);
    });
}
function createSequences(db) {
    return __awaiter(this, void 0, void 0, function* () {
        const now = Date.now();
        const sequences = [
      ...
      `, [
        ...
        yield new Promise((resolve) => {
            db.get('SELECT COUNT(*) as count FROM sequence_stats', [], (err, result) => {
                if (err) {
...
function updateHourlyStats(db, date, hour, trades) {
    return __awaiter(this, void 0, void 0, function* () {
...
function createTrades(date, hour, count, winRate) {
    const trades = [];
